...
void test_add_one_particle() {
    ...
    TEST_ASSERT_EQUAL(10, ps->firstP->p.pos.x);
    TEST_ASSERT_EQUAL(10, ps->firstP->p.pos.y);
...
// from particlesystem.c
int collides_with_black_hole(Particlesystem* ps, Particle* p);

void test_collides_with_black_hole_returns_0_for_black_hole_itself() {
    Particle black_hole = new_particle(100, 100, new_color(0,0,0));

    add_black_hole(ps, black_hole);

    int res = collides_with_black_hole(ps, &ps->firstP->p);
...
int main(void) {
    ...
    RUN_TEST(test_collides_with_black_hole_returns_0_for_black_hole_itself);
    return UNITY_END();
}